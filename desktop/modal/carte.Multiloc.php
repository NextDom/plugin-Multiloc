    ...
    foreach ($eqLogic->getCmd('info') as $cmd) {
        if ($cmd->getEqLogic_id() == $id) {
            if ($cmd->getConfiguration("Typeloc") == "personne") {
                ...
                $person = $person . 'L.marker([' . $cmd->getConfiguration("position") . '], {icon: icon' . $cmd->getName() . '}).addTo(mapmodal' . $cmd->getEqLogic_id() . ').bindPopup("' . $cmd->getName() . '"); ';
...
<script>
    var mapmodal<?php echo $id ?> = L.map("mapmodal<?php echo $id ?>").setView([<?php echo $map_center ?>], <?php echo $zoom ?>);
    <?php echo $icon ?>
